...
<html>
	...
	<body>
		<div class="w content">  <!-- border #DEDEDE -->
			...
			<div class="header" >
				<span class="word"><strong>未完成实训报告</strong></span>
			...
			<div class="tables">
				<table class="tab" border="1px" cellspacing="0" cellpadding="0">
					...
					<c:forEach var="training" items="${requestScope.list }">
						<c:if test="${!empty training }">
							<tr>
								<td>${training.courseName }</td>
								<td>${training.trainingName }</td>
								<td>${training.teacherName }</td>
								<td>${training.trainingWeek }</td>
								<td>${training.trainingFinishtime }</td>
								<td>
